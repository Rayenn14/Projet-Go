...
public class Board implements IBoard {
    ...
    public String findBestMove(PawnType playerPawn) {
        ...
        if (boardIsEmpty) {
            int centerRow = getBoardSize() / 2;
            int centerCol = getBoardSize() / 2;
            return String.valueOf((char)('A' + centerCol)) + (centerRow + 1);
        }

        for (int row = 0; row < getBoardSize(); row++) {
            for (int col = 0; col < getBoardSize(); col++) {
                int index = row * getBoardSize() + col;

                if (getboard()[index] == null) {
                    if (!hasAdjacentPawn(row, col)) continue;

                    // Simuler le placement du pion
                    getboard()[index] = new Pawn(playerPawn);
    ...
    private boolean hasAdjacentPawn(int row, int col) {
        ...
        for (int[] dir : directions) {
            ...
            if (newRow >= 0 && newRow < getBoardSize() &&
                    newCol >= 0 && newCol < getBoardSize()) {
                int index = newRow * getBoardSize() + newCol;
                if (getboard()[index] != null) {
                    return true;
                }
            }
    ...
    private int evaluateOpponentBestMove(PawnType playerPawn) {
        ...
        for (int oppRow = 0; oppRow < getBoardSize(); oppRow++) {
            for (int oppCol = 0; oppCol < getBoardSize(); oppCol++) {
                int oppIndex = oppRow * getBoardSize() + oppCol;

                // Trouver un coup possible pour l'adversaire
                if (getboard()[oppIndex] == null) {
                    getboard()[oppIndex] = new Pawn(opponentPawn);
